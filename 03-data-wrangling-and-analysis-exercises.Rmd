...
nhanes %>% 
    ...
    select(pct_days_phys_health_bad, days_phys_hlth_bad) %>% 
    mutate(pct_formatted = percent(pct_days_phys_health_bad,
                                   accuracy = 1))
```
...
Create a pipeline that keeps only those are college grads (line #1). Then, `filter` to keep only those who are divorced, separated, or widowed. Finally, use `select` to show only the `education` and `marital_status` variables.
...
Create a pipeline that starts with your `nhanes` data frame, does the following, and saves the result to a new data frame called `mental_health_over_30`:
